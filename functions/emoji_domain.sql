-- Postgres has no CREATE DOMAIN IF NOT EXISTS / OR REPLACE, and employees.emoji
-- already depends on this domain, so a DROP...CASCADE guard would silently drop
-- that column on redeploy. Skip creation if it already exists instead.
-- Trade-off: since this skips whenever the domain already exists, editing the
-- CHECK below won't apply on redeploy to an environment that already has it —
-- that requires an explicit ALTER DOMAIN or a manual drop/recreate.
DO $$
BEGIN
