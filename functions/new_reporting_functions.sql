...
$$
    SELECT
        ...
        d::date AS work_day
    FROM employees e
    CROSS JOIN generate_series(from_date, to_date, '1 day'::interval) d
    WHERE is_weekday_new(d::date)
        AND NOT is_holiday(d::date)
        AND is_employed_at_date(e.id, d::date)
$$ LANGUAGE sql;
...
$$
    WITH staffing_hours AS (
        ...
        WHERE s.date BETWEEN from_date AND to_date
            AND is_employed_at_date(s.employee, s.date)
    )
    SELECT
